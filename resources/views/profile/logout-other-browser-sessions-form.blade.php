<x-jet-action-section>
    <x-slot name="title">
        {{ __('fields.profile.session.title') }}
    </x-slot>

    <x-slot name="description">
        {{ __('fields.profile.session.desc') }}
    </x-slot>
    ...
    <x-slot name="content">
        ...
        <div>
            {{ __('fields.profile.session.text') }}
        </div>
                            ...
                            <div>
                                <div class="small font-weight-lighter text-muted">
                                    ...
                                    @if ($session->is_current_device)
                                        <span class="text-success font-weight-bold">{{ __('fields.profile.session.this') }}</span>
                                    @else
                                        {{ __('fields.profile.session.last') }} {{ $session->last_active }}
                                    @endif
                                </div>
                            </div>
                        </div>
        ...
        <div class="d-flex mt-3">
            <x-jet-button wire:click="confirmLogout" wire:loading.attr="disabled">
                {{ __('fields.profile.session.button') }}
            </x-jet-button>
        </div>
